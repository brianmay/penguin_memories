defmodule PenguinMemories.Database.Updates do
  ...
  defp apply_updates_to_object(updates, object) do
    ...
    case PenguinMemories.Database.Query.apply_edit_changeset(changeset) do
      {:ok, new_obj} ->
        {:ok, changeset, new_obj}

      {:error, _changeset, error_message} ->
        {:error, "Update of object #{object.id} failed: #{error_message}"}
    end
  end
  ...
  defp apply_standard_bulk_updates(updates, query, backend) do
    Repo.transaction(fn ->
      ...
      |> Stream.each(fn result ->
        ...
        if object_id do
          # Get the schema module from the backend and fetch the full struct
